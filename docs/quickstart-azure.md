...
The deploying principal requires **`Application.ReadWrite.All`** Microsoft Graph permission. This can be granted via the **Application Administrator**, **Cloud Application Administrator**, or **Application Developer** Entra ID role. See the [CI/CD Setup Guide](design/cicd.md) for details.
...
| Deployment fails with Graph permission error | The deploying principal needs `Application.ReadWrite.All`. See [CI/CD Setup Guide](design/cicd.md). |
...
* [CI/CD Setup Guide](design/cicd.md) for GitHub Actions pipeline configuration
